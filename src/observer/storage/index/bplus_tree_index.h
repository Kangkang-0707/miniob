...
private:
  RC   make_key(const char *record, vector<char> &key) const;
  int  key_length() const;
  bool is_single_field_index() const { return field_metas_.size() == 1; }
  bool has_null_field(const char *record) const;
