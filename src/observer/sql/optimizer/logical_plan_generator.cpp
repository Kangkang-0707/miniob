...
RC LogicalPlanGenerator::create_plan(SelectStmt *select_stmt, unique_ptr<LogicalOperator> &logical_operator)
{
  ...
  RC                          rc = RC::SUCCESS;
  if (select_stmt->predicate_expression()) {
    predicate_oper = make_unique<PredicateLogicalOperator>(std::move(select_stmt->predicate_expression()));
  }
  ...
  if (!select_stmt->order_by().empty()) {
    order_by_oper =
        make_unique<OrderByLogicalOperator>(std::move(select_stmt->order_by()), std::move(select_stmt->order_ascs()));
    if (*last_oper) {
...
RC LogicalPlanGenerator::create_plan(UpdateStmt *update_stmt, unique_ptr<LogicalOperator> &logical_operator)
{
  ...
  unique_ptr<LogicalOperator> update_oper(new UpdateLogicalOperator(
      table, update_stmt->field_metas(), update_stmt->values(), update_stmt->value_sub_queries()));
